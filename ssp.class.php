...
class SSP {
	...
	static function simple($request, $sql_details, $table, $primaryKey, $columns, $default = false)
	{
		...
		$data = self::sql_exec(
			$db,
			$bindings,
			"SELECT SQL_CALC_FOUND_ROWS `" . implode("`, `", self::pluck($columns, 'db')) . "` FROM `$table` $where $order $limit"
		);
		...
		$resTotalLength = self::sql_exec($db, "SELECT COUNT(`{$primaryKey}`) FROM `$table`" . ($default !== false ? ' WHERE ' . $default : ''));
	 ...
	 * @param  array $bindings Array of PDO binding values from bind() to be used for safely escaping strings. Note that this can be given as the SQL query string if no bindings are required.
